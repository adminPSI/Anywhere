...
namespace Anywhere.service.Data
{
    public class AssessmentReportSQL
    {
        ...
        public DataSet ISPSummary(long AssesmentID, bool Assessment, string WhichISPArea)
        {
            sb.Clear();
            sb.Append("SELECT  DBA.anyw_isp_consumer_assessment_answers.answer, DBA.anyw_isp_assessment_sections.section_title, ");
            sb.Append("DBA.anyw_isp_assessment_sections.section_order, DBA.anyw_isp_assessment_subsections.subsection_title, ");
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (row["question_text"].ToString().Equals("Who is responsible:"))
                {
                    if (row["answer"].ToString() != string.Empty)
                    {
                        ...
                        {
                            sb.Clear();
                            sb.Append("SELECT  DBA.Vendor.Name ");
                            sb.Append("FROM  DBA.Vendor ");
                            sb.AppendFormat("WHERE DBA.Vendor.Vendor_ID = {0} ", row["answer"].ToString().ToUpper().Replace("V", ""));
                            DataRow row2 = di.SelectRowsDS(sb.ToString()).Tables[0].Rows[0];
        ...
        public DataSet ISPOutcomes(long AssesmentID)
        //Added Who2 for new People Name *********************************************************************************
        {
            ...
            sb.Append("People_1.Last_Name AS ContactLastName, People_1.First_Name AS ContactFirstName, DBA.Vendor.Name AS ProviderName, ");
            ...
            sb.Append("LEFT OUTER JOIN DBA.Vendor ON DBA.ANYW_ISP_Outcome_Experience_Responsibility.Responsible_Provider = DBA.Vendor.Vendor_ID ");
        ...
        public DataSet ISPServices(long AssesmentID)
        {
            sb.Clear();
            ...
            sb.Append("DBA.ANYW_ISP_Services_Paid_Support.Row_Order, DBA.Service_Types.Description, DBA.Funding_Source_Info.Description AS FundingSource, ");
            sb.Append("DBA.Vendor.Name AS Provider, DBA.anyw_isp_assessment_sections.section_title AS Area, DBA.ANYW_ISP_Services_Paid_Support.Funding_Source, ");
            ...
            sb.Append("LEFT OUTER JOIN DBA.Vendor ON DBA.ANYW_ISP_Services_Paid_Support.Provider_ID = DBA.Vendor.Vendor_ID ");
            sb.Append("LEFT OUTER JOIN DBA.Funding_Source_Info ON DBA.ANYW_ISP_Services_Paid_Support.Funding_Source = DBA.Funding_Source_Info.Funding_Source_ID ");
        ...
        public DataSet ISPTeamMembers(long AssesmentID)
        {
            sb.Clear();
            ...
            sb.Append("FROM DBA.Vendor ");
            sb.Append("RIGHT OUTER JOIN DBA.ANYW_ISP_Consumer_Informed_Consent ON DBA.Vendor.Vendor_ID = DBA.ANYW_ISP_Consumer_Informed_Consent.CS_Contact_Provider_Vendor_ID ");
        ...
        public DataSet ISPTeamMembers2(long AssesmentID)
        {
            sb.Clear();
            sb.Append("SELECT   dba.ANYW_ISP_Signatures.ISP_Consumer_Signature_ID, dba.Vendor.Name, dba.ANYW_ISP_Signatures.ISP_Consumer_Plan_ID, ");
            ...
            sb.Append("LEFT OUTER JOIN dba.Vendor ON dba.ANYW_ISP_Signatures.CS_Contact_Provider_Vendor_ID = dba.Vendor.Vendor_ID ");
        ...
        public DataSet ISPContacts(long AssesmentID)
        {
            sb.Clear();
