    ...
    public class AssessmentReportSQL
    {
        ...
        public DataSet AssesmentAnswers(long AssesmentID, bool Assessment)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                try
                {
                    DataRow[] drSC = SCdt.Select("isp_assessment_subsection_id = " + row["isp_assessment_subsection_id"]);

                    if (drSC != null)
                    {
                        if (drSC.Count() > 0)
                        {
                            if (Convert.ToInt32(drSC[0]["Counter"]) > 0)
                            {
                                row["SubSectionCount"] = Convert.ToInt32(drSC[0]["Counter"]);
                            }
                        }
                    }
                }
                ...
                try
                {
                    ...
                    DataRow[] drHSC = HSCdt.Select(sb.ToString());
                    ...
                    if (drHSC != null)
                    {
                        if (drHSC.Count() > 0)
                        {
                            if (Convert.ToInt32(drHSC[0]["iCount"]) > 0)
                            {
        ...
        public DataSet ISPTeamMembers2(long AssesmentID, Boolean Advisor = false)
        {
            if (Advisor == true)
            {
                ...
                sb.Append("FROM dba.ANYW_ISP_Consumer_Plans ");
                sb.Append("LEFT OUTER JOIN dba.ANYW_ISP_Signatures ON dba.ANYW_ISP_Consumer_Plans.isp_consumer_plan_id = dba.ANYW_ISP_Signatures.ISP_Consumer_Plan_ID ");
            ...
            else if (Advisor == false)
            {
                sb.Clear();
                sb.Append("SELECT ");
                sb.Append("sig.ISP_Consumer_Signature_ID, ");
                sb.Append("sig.ISP_Consumer_Plan_ID, ");
                sb.Append("sig.CS_Change_Mind, ");
                sb.Append("sig.CS_Change_Mind_SSA_People_ID, ");
                sb.Append("sig.CS_Contact, ");
                sb.Append("sig.CS_Contact_Provider_Vendor_ID, ");
                sb.Append("sig.CS_Contact_Input, ");
                sb.Append("sig.CS_Rights_Reviewed, ");
                sb.Append("sig.CS_Agree_To_Plan, ");
                sb.Append("sig.CS_FCOP_Explained, ");
                sb.Append("sig.CS_Due_Process, ");
                sb.Append("sig.CS_Residential_Options, ");
                sb.Append("sig.CS_Supports_Health_Needs, ");
                sb.Append("sig.Name AS SupportName, ");
                sb.Append("SSA.Last_Name, ");
                sb.Append("SSA.First_Name, ");
                sb.Append("SSA.Middle_Name, ");
                sb.Append("sig.Signature_Order, ");
                sb.Append("sig.CS_Technology AS TechSolutionsExplored, ");
                sb.Append("SSA.First_Name + ' ' + SSA.Last_Name AS Name2, ");
                sb.Append("org.Name AS OrganizationName, ");
                sb.Append("sig.ISP_Signature_Type, ");
                sb.Append("plans.Summary_of_Changes, ");
                sb.Append("plans.effective_start, ");
                sb.Append("plans.plan_type, ");
                sb.Append("plans.revision_number ");
                sb.Append("FROM dba.ANYW_ISP_Consumer_Plans plans ");
                sb.Append("LEFT JOIN dba.ANYW_ISP_Signatures sig ");
                sb.Append("ON plans.isp_consumer_plan_id = sig.ISP_Consumer_Plan_ID ");
                sb.Append("AND sig.ISP_Signature_Type < 3 ");
                sb.Append("AND sig.Team_Member IN ('Parent/Guardian', 'Guardian', 'Person Supported') ");
                sb.Append("LEFT JOIN dba.People SSA ");
                sb.Append("ON sig.CS_Change_Mind_SSA_People_ID = SSA.ID ");
                sb.Append("LEFT JOIN dba.Organization org ");
                sb.Append("ON SSA.Organization_ID = org.Organization_ID ");
                sb.AppendFormat("WHERE plans.isp_consumer_plan_id = {0} ", AssesmentID);
            }
