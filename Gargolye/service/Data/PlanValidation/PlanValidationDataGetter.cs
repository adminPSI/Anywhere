...
{
    public class PlanValidationDataGetter
    {
        private static Loger logger = new Loger();
        private string connectString = ConfigurationManager.ConnectionStrings["connection"].ToString();

        public string getContactValidationData(string token, string planId)
        {
            ...
            list.Add(token);
            list.Add(planId);
            string text = "CALL DBA.ANYW_GetContactValidationData(" + string.Join(",", list.Select(x => string.Format("'{0}'", removeUnsavableNoteText(x))).ToList()) + ")";
            try
            {
            ...
            {
                logger.error("1CNR", ex.Message + "ANYW_GetContactValidationData(" + string.Join(",", list.Select(x => string.Format("'{0}'", x)).ToList()) + ")");
                return "1CNR: error ANYW_GetContactValidationData";
            }
        }
