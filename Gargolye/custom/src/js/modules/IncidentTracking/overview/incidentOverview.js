const incidentOverview = (function () {
    ...
    let retrieveData = {
        ...
        fromDate: '',
        toDate: '',
        viewCaseLoad: $.session.incidentTrackingCaseLoad,
    };
    let emailString;
    ...
    function buildFilterPopup(IsShow) {
        ...
        groupDropdown = dropdown.build({
            ...
            label: 'Group',
            style: 'secondary',
            readonly: $.session.incidentTrackingCaseLoad,
        });
    ...
    function filterDropdownEventSetup() {
        ...
        alphaDropdown.addEventListener('change', event => {
            ...
            retrieveData.employeeId = '%';
            retrieveData.subcategoryId = '%';
            retrieveData.viewCaseLoad = $.session.incidentTrackingCaseLoad;
            // temp cache data
    ...
    async function populateGroupsDropdown(selectedLocationId) {
        ...
        if ($.session.incidentTrackingCaseLoad) {
            dropdown.populate(groupDropdown, data, 'CAS-0');
        } else {
    ...
    async function getConsumerDropdownData() {
        const getConsumerByGroupData = {
            ...
            selectedActive: 'No', 
            isCaseLoad: $.session.incidentTrackingCaseLoad ? true : false
        };
    ...
    function init() {
        ...
        retrieveData.groupId = '%';
        groupIdSelected = '%';
        groupCodeSelected = $.session.incidentTrackingCaseLoad ? 'CAS' : 'All';
        locationIdSelected = '0';
        filterData.groupName = $.session.incidentTrackingCaseLoad ? 'Caseload' : 'EVERYONE';
