var outcomes = (function() {
  ...
  function updateCurrentFilterDisplay(service = "All", outcomeType = "All") {
    ...
    <p><span>Service: </span>${service} </p> 
    <p><span>Outcome Type: </span>${outcomeType}</P>
    `
  }
  ...
  function setupFilterEvents() {
    ...
    applyButton.addEventListener("click", () => {
      ...
      let outcomeTypeText = outcomeDd.options[outcomeDd.selectedIndex].innerHTML
  ...
  function buildCardDetails(editData) {
    ...
    if (editData) {
      ...
      lastEditedTime = `${lastEditHH}:${lastEditMM} ${
        editData.Last_Update.split(" ")[2]
      }`;
  ...
  function buildCardOverview(outcome) {
    ...
    if (lastUpdateBy !== "") {
      ...
      <p>Last update by ${lastUpdateBy.split(" ")[0]} ${
        lastUpdateBy.split(" ")[1]
      }</p>
 ...
 function dashHandler(myconsumerId) {
  ...
  function loadCardView(selectedConsumer) {
    ...
    //Filter button
    var filterBtn = document.querySelector(".outcomesFilter");
    if (!filterBtn) {
      filterButton = buildFilterBtn();
      topFilterDateWrap.appendChild(filterButton);
    }
    //Fucking Date Input
    ...
    if (!document.querySelector(".currentFilterDisplayOC")) {
