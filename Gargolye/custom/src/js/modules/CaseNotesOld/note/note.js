var note = (function () {
  ...
  let locationDropdown, serviceDropdown, serviceLocationDropdown, needDropdown, vendorDropdown, contactDropdown;
  let myPhrasesBtn,
    deleteNoteBtn,
    cancelNoteBtn,
    timerStartBtn,
  ...
  function setNoteValues(reviewData) {
    if (pageLoaded === 'new') {
      ...
      serviceMonitoring = rd.outcomeServiceMonitoring;
  ...
  async function noteSaveUpdate(saveAndNew) {
    ...
    if (pageLoaded === 'new') {
      ...
      var saveData = {
        ...
        reviewRequired,
        serviceMonitoring, // from checkbox
      };
      newNote.saveNote({ overlapData, saveData, selectedConsumerIds, saveAndNew }, tempAttachmentArray);
      return;
    }
    ...
    if (pageLoaded === 'review') {
      ...
      var updateData = {
        ...
        consumerId,
        serviceOrBillingCodeId: serviceCode,
        locationCode: locationInput.selectedIndex === -1 ? 0 : locationInput.options[locationInput.selectedIndex].value,
        serviceCode: serviceInput.selectedIndex === -1 ? 0 : serviceInput.options[serviceInput.selectedIndex].value,
        needCode: needInput.selectedIndex === -1 ? 0 : needInput.options[needInput.selectedIndex].value,
        serviceDate,
        startTime,
        endTime,
        vendorId,
        contactCode: contactInput.selectedIndex === -1 ? 0 : contactInput.options[contactInput.selectedIndex].value,
        serviceLocationCode: serviceLocationId,
        reviewRequired,
        confidential,
        corrected,
  ...
  function dealWithDropdownDataHugeString(res) {
    function decipherXML(res) {
      var xmlDoc,
        data = [];
      xmlDoc = UTIL.parseXml('<?xml version="1.0" encoding="UTF-8"?>' + res.populateDropdownDataResult);
      ...
      billingCodes.forEach(bc => {
        ...
        contacts.forEach(loc => {
          var tmpContactName = [].slice.call(loc.getElementsByTagName('contactname'))[0].textContent;
          var tmpContactCode = [].slice.call(loc.getElementsByTagName('contactcode'))[0].textContent;
        ...
        services.forEach(loc => {
          var tmpServiceName = [].slice.call(loc.getElementsByTagName('servicename'))[0].textContent;
          var tmpServiceCode = [].slice.call(loc.getElementsByTagName('servicecode'))[0].textContent;
  ...
  function checkServiceFunding() {
    if ($.session.applicationName === 'Advisor') {
      if (
        serviceCodeDropdown.firstChild.options[serviceCodeDropdown.firstChild.selectedIndex].getAttribute(
        ...
        if (serviceLocationId === '' || serviceLocationId === '00') serviceLocationDropdown.classList.add('error');
      }
    }
  }
  ...
  function populateVendorDropdown() {
    caseNotesAjax.getConsumerSpecificVendors(selectedConsumerIds[0], serviceDate, function (results) {
  ...
  function populateServiceLocations() {
    ...
    caseNotesAjax.getServiceLocationsForCaseNoteDropDown(serviceLocData, function (results) {
      ...
      if ($.session.applicationName === 'Advisor') dropdown.populate(serviceLocationDropdown, data, defaultVal);
    });
  }
  ...
  function moveConsumerToConsumerSection() {
    ...
    if (pageLoaded === 'review') {
      ...
      if (allowGroupNotes) {
        ...
        const selectedConsumerName = activeConsumers[activeConsumers.length - 1].card.innerText.replace(/\s/g, '');
  ...
  function buildDocTime() {
    ...
    var docTimeMinVal = rd ? (rd.totaldoctime ? Math.round(Math.floor(parseInt(rd.totaldoctime) / 30) / 2) : 0) : 0;
  ...
  function buildCardDetailsSection() {
    async function saveBtnAction(saveAndNew) {
      ...
      if ((tempAttachmentArray.length > 0 || reviewAttachmentArray.length > 0) && convertToGroupNote) {
    ...
    serviceMonitoringCheckbox = input.buildCheckbox({
      text: 'Outcome/Service Monitoring',
      isChecked: serviceMonitoring === 'Y' ? true : false,
    });
  ...
  function buildCaseNoteCard() {
    ...
    cardHeader.innerHTML = pageLoaded === 'new' ? `<h3>New Note</h3>` : `<h3>Note Review - ${consumerName}</h3>`;
  ...
  function docTimeRequiredPopup() {
    ...
    popupText.innerHTML = 'Documentation Time is allowed for this bill code. Would you like to start the timer now?';
  ...
  function checkADVRequiredFields(calledAction) {
    ...
    serviceCode = serviceCodeDropdownOptions.options[serviceCodeDropdownOptions.selectedIndex].value;
    if (serviceCode === '') {
  ...
  function checkGKRequiredFields(calledAction) {
    ...
    if (docTimeRequired === 'Y' && calledAction !== 'consumerSelect' && (cnBatched === '' || cnBatched === null)) {
  ...
  function setupCardEvents() {
    ...
    serviceMonitoringCheckbox.addEventListener('change', event => {
      serviceMonitoring = event.target.checked ? 'Y' : 'N';
    });
  ...
  function setInputstoReadOnly() {
    ...
    if ($.session.applicationName === 'Gatekeeper') serviceMonitoringCheckbox.firstChild.setAttribute('disabled', true);

    if (docTimeMinutesField) docTimeMinutesField.querySelector('.input-field__input').setAttribute('readonly', true);
  ...
  function removeAttachmentFromTempAttachmentArray(attachmentName) {
    tempAttachmentArray = tempAttachmentArray.filter(attachment => attachment.description !== attachmentName);
  }
