const TRANS_manageRoutes = (function () {
  ...
  function buildPage(resetFilter) {
    ...
    routesTable = table.build({
      ...
      headline: "Routes",
      columnHeadings: ["Date", "Route", "Driver", "Vehicle", "Location", "Start Odometer", "End Odometer", "Start Time", "Stop Time"],
    });
  ...
  function populateTable() {
    ...
    filteredRoutes.forEach((val,key,map) => {
      const { batchNumber, dateOfService, tripId, tripName, vehicleInfoId, odoStart, odoStop, startTime, endTime, locationName, driverId, tripInspection, tripsCompletedId } = val;
      ...
      const rowValues = [readDate, tripName, driverDisp, vehicleNumber, locationName, readOdoStart, readOdoStop, readStartTime, readEndTime];
      tableData.push({
        values: rowValues,
        id: key,
  ...
  function buildFilteredBy() {
    ...
    const filteredDateStart = UTIL.abbreviateDateYear(UTIL.formatDateFromIso(filterOpts.serviceDateStart, '/'));
    const filteredDateStop = UTIL.abbreviateDateYear(UTIL.formatDateFromIso(filterOpts.serviceDateStop, '/'));
    const selectedDriver = driverLookup ? `${driverLookup.Last_Name}, ${driverLookup.First_Name}` : 'All';
  ...
  async function getData() {
    ...
    tmpRoutes.forEach(route => {
      if (locationPermissions.has(route.locationId)) {
        ...
        console.info(`skipping route ${route.tripsCompletedId}. User does not have permission to the route's location.`)
      }
    })
  ...
  function enableMultiEditRows() {
    ...
    const highlightedRows = [].slice.call(
