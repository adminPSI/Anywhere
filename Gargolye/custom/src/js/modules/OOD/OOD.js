const OOD = (() => {
	...
	async function buildOODEntriesTable(filterValues) {
		...
		const { getOODEntriesResult: OODEntries } = await OODAjax.getOODEntriesAsync(
			...
			OODEntries.forEach(function (entry) {
				...
				let formatServiceDate = UTIL.leadingZero(theMonth) + '/' + UTIL.leadingZero(newDate.getDate()) + '/' + newDate.getFullYear();
			...
			let tableData = OODEntries.map((entry) => ({
				...
				if (rowConsumer[0] && e.target.attributes.OODReportType.value === 'newEntry' && e.target.attributes.serviceType.value === 'T1' ) {
				...
				if (rowConsumer[0] && e.target.attributes.OODReportType.value === 'monthlySummary' && e.target.attributes.serviceType.value === 'T1' ) {
					OODAjax.getForm4MonthlySummary(e.target.attributes.Id.value, function (results) {
					OODForm4MonthlySummary.init(results, rowConsumer[0], undefined, e.target.attributes.userId.value);
					});
				} 
    ...
    function buildConsumerCard(consumer) {
		...
		consumerCard.addEventListener('click', (event) => { 
			...
			selectedConsumers = selectedConsumers.filter(function (x) {return x.id !== consumerId } );
	  ...
	  function buildButtonBar(consumer) {
		...
		const entryBtn = button.build({
		  ...
		  callback: () => buildEntryServicePopUp(entryBtn.attributes.consumerid.value, entryBtn.attributes.btnType.value),
		});
		const summaryBtn = button.build({
		  text: 'New Monthly Summary',
		  style: 'secondary',
		  type: 'contained',
		  attributes: [{ key: 'consumerId', value: consumer.id }, { key: 'btnType', value: 'monthlySummary'}],
		  classNames: !$.session.OODInsert ? ['newMonthlySummaryBtn','disabled'] : ['newMonthlySummaryBtn'],
		  callback: () => buildSummaryServicePopUp(summaryBtn.attributes.consumerid.value, summaryBtn.attributes.btnType.value),
			
		});
	
		buttonBar.appendChild(entryBtn);
		buttonBar.appendChild(summaryBtn);
		...
		if (consumerServices === undefined || consumerServices.length == 0) {
			...
			consumerServicesDoneBtn = button.build({
				...
				function serviceDateInputRequired() {
					...
					if (!serviceDate || serviceDate === CLEAREDSERVICEDATE || dates.isAfter(thisServiceDate, todayDate)) {
  ...
  function buildNoServicesPopup(consumerId, btnType) {
	...
	let OKBtn = button.build({
	  ...
	  warningMessage.innerHTML = 'There are no authorized services for the selected consumer. Please contact your Advisor OOD Administrator to enter an authorization or select a different consumer.';
	  btnWrap.classList.add("btnWrap");
...
function buildEditEmployersBtn() {
	...
	return button.build({
	  ...
	  id: editEmployersBtn,
	 //classNames: !$.session.formsInsert ? ['disabled'] : ['newPlanBtn'],
	  classNames: ($.session.OODUpdate || $.session.OODInsert || $.session.OODView) ? ['editEmployersBtn'] : ['editEmployersBtn','disabled'],
	 callback: async () => { OODEmployers.init()}, 
	});
  }
	  ...
	  function buildFilteredBy() {
		...
		filteredBy.innerHTML = `
		  <div class="filteredByData">
			...
			<p><span>Reference Number:</span> ${(filterValues.referenceNumber == '%') ? 'ALL' : filterValues.referenceNumber}</p>
		  </div>
		`;
	...
	async function populateEmployeeDropdown() { 
		...
		} = await OODAjax.getActiveEmployeesAsync();
  ...
  async function populateReferenceNumberDropdown() { 
	...
	async function populateConsumerServiceCodeDropdown(consumerId, serviceDate) { 
		...
		const {
			getConsumerServiceCodesResult: services,
		} = await OODAjax.getConsumerServiceCodesAsync(consumerId, serviceDate);
	  // const templates = WorkflowViewerComponent.getTemplates();
