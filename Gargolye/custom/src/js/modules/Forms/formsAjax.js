var formsAjax = (function () {
        ...
        WebViewer(
          {
            ...
            licenseKey:
              'Marshall Information Services, LLC(primarysolutions.net):OEM:Advisor/Anywhere;Gatekeeper/Anywhere::B+:AMS(20220512):A8A5354D0437C60A7360B13AC9A2537860614FABB956CD3BD5343BC2C76C38C054C2BEF5C7',
            documentType: 'pdf',
          ...
          instance.setHeaderItems(header => {
            header.push({
              ...
              onClick: async () => {
                ...
                const result = await WorkflowViewerAjax.insertStepDocumentAsync(
                  stepId,
                  docOrder,
                  templateName + '.pdf',
                  'pdf',
                ...
                const {
                  insertWorkflowStepDocumentResult: { attachmentId, documentId },
                } = result;
                ...
                const stepsContainer = document.querySelector(
                  `.wf-steps-container[data-id='${stepId}']`,
                );
            ...
            header.push({
              ...
              onClick: async () => {
                try {
                  ...
                  let documentEdited = '1'; // documentEdited = T
                  const result = await WorkflowViewerAjax.updateStepDocumentAsync(
                    documentId,
                    'pdf',
                    data,
                    documentEdited,
                  ...
                  const {
                    updateWorkflowStepDocumentResult: { attachmentId, documntId },
                  } = result;

                  alert('Document has been saved.');

                  POPUP.hide(formPopup);
                  const updatedDocument = document.querySelector(
            ...
            if (documentEdited == '1') {
              header.push({
                ...
                onClick: async () => {
                  ...
                  let isRefresh = true;
                  const result = await formsAjax.getPDFDocData(
                    documentId,
                    '0',
                    consumerId,
                    isRefresh,
              ...
              header.push({
                ...
                onClick: async () => {
                  try {
                    ...
                    if (documentEdited === '1') {
                      const result = await formsAjax.updateConsumerFormAsync(
                        formId,
                        data,
                        documentEdited,
                      );
                      const {
                        updateConsumerFormResult: { formID },
                      } = result;
                    } else {
                      const result = await formsAjax.insertConsumerFormAsync(
                        $.session.UserId,
                        consumerId,
                        formId,
                        data,
            ...
            if (isTemplate == '0' && $.session.formsUpdate == true) {
              header.push({
                ...
                onClick: async () => {
                  ...
                  // get the data for annotated version of the document
                  let isRefresh = true;
                  const result = await formsAjax.getPDFFormData(
                    formId,
                    '0',
                    consumerId,
                    isRefresh,
  ...
  async function updateConsumerFormAsync(formId, formdata, documentEdited) {
    try {
      var binary = '';
      var bytes = new Uint8Array(formdata);
      var len = bytes.byteLength;
  ...
  async function updateConsumerFormCompletionDateAsync(formId, formCompletionDate) {
    try {
      const result = await $.ajax({
        type: 'POST',
        url:
    ...
    try {
      const result = await $.ajax({
        ...
        url:
          ...
          $.webServer.serviceName +
          '/deleteConsumerForm/',
        data: '{"token":"' + $.session.Token + '", "formId":"' + formId + '"}',
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
      });
      return result;
  ...
  function documentNotAllowedSaveAlert() {
    ...
    warningMessage.innerHTML =
      'This PDF document format does not allow saving. Any entered data can not be saved to the database.';
  ...
  function closewarningPopup(formId, documentEdited, consumerId, isRefresh, isTemplate) {
    ...
    var warningMessage = document.createElement('p');
    warningMessage.innerHTML =
      'You should not close this form until you have saved your work. Do you want to proceed with closing this form?';
