class WorkflowComponent {         
    ...
    render() {
                ...
                try {
                    ...
                    for (const ddl in ddlcache) {   
                           if (ddlcache[ddl].modified != '') { 
                              ...
                              let peopleId = await WorkflowViewerAjax.updateRelationshipResponsiblePartyID(ddlcache[ddl].modified, workflowId, workflowResponsibilityType);               
                              ...
                              for (var i = 0; i < workflowSteps.length; i++) {
                                if (workflowSteps[i].getElementsByClassName(`responsiblePartytype_${workflowResponsibilityType}`)[0] != undefined) {
                                    if (workflowSteps[i].getElementsByClassName("wf-step-done-date")[0].innerText == "") {
                                        let responsiblestep = workflowSteps[i].getElementsByClassName(`responsiblePartytype_${workflowResponsibilityType}`)[0];
                                        responsiblestep.getElementsByTagName("p")[0].innerText = getFirstAndLastName(ddlcache[ddl].modified, people);  
                                    }
                                }
        ...
        statusDropdown.addEventListener("change", async (e)=> {
            ...
            let cache = {original : currentStatusId, modified : e.target.value, isChanged : changed, eventTypeId : workflowId, eventType : "workflow", eventId : 12};
        ...
        dropdown.populate(statusDropdown, statuses.map( s => ({ id: s.statusId, value: s.statusId, text: s.description})), currentStatusId);       
