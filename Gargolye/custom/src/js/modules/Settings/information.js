const information = (function () {
    ...
    async function getValidateEmailInformation() {
        try {
            const data = await $.ajax({
                type: 'POST',
                url:
            ...
            return data.GetValidateEmailInformationResult[0];
        } catch (error) {
            console.log(error);
        }
    }
    ...
    async function emailValidation() {
        const result = await getValidateEmailInformation();
        if (result.isEmailExist == 0 || result.isEmailExist == '0') {
            await updateStaffDemographicInformation();
            await updateInformationData();
        }
        else {
            emailDuplicationPopUp();
        }
    }


    function emailDuplicationPopUp() {
        const emailDuplicationPOPUP = POPUP.build({
            hideX: true,
        });
        const okBtn = button.build({
            ...
            type: 'contained',
            callback: () => {
                POPUP.hide(emailDuplicationPOPUP);
            },
        });
        ...
        heading.innerText = 'Duplicate email address'; 


        const message = document.createElement('p');
        message.innerText = 'This email address is already in use, please enter a unique address to save.';
        message.style.textAlign = 'center';
        message.style.marginBottom = '15px';
        emailDuplicationPOPUP.appendChild(heading);
        emailDuplicationPOPUP.appendChild(message);
        emailDuplicationPOPUP.appendChild(okBtn);
        okBtn.focus();
        POPUP.show(emailDuplicationPOPUP);
    }
    ...
    function buildPage() {
        ...
        const updateButton = button.build({
            ...
            //    await updateInformationData();
            //},
            callback: async () => { await emailValidation(); },
