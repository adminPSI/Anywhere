  ...
  var selectedEnd;
	...
	function populateSchedule(filterBy) {
		...
		dates.forEach(date => {
			...
			times.sort().forEach(timeFrame => {
				...
				var endTime = moment()
					.hour(parseInt(end[0], 10))
					.minute(parseInt(end[1], 10));
				...
				locations.forEach(location => {
					...
					div.innerHTML += `
            <p>${startTime.format('h:mma')} to ${endTime.format('h:mma')} - ${location}${initials !== '' ? ` - ${initials}` : ''}</p>
          `;
				});
			});
	...
	function populateLocationDropdown() {
      ...
      $.session.myscheduleWidgetLocationText = data[0].value;
		...
		dropdown.populate('dashboardScheduleLocationDropdown', data, $.session.myscheduleWidgetLocationText);
	}
    ...
    if (!defaultValue) {
      ...
      selectedStart = data[0].start;
      selectedEnd = data[0].end;
    }
