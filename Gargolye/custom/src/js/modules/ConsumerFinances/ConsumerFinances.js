const ConsumerFinances = (() => {
    ...
    async function buildConsumerFinanceEntriesTable(filterValues) {
        ...
        let tableData = ConsumerFinancesEntries.getAccountTransectionEntriesResult.map(entry => {
            ...
            return {
                ...
                attributes: [
                    { key: 'registerId', value: entry.ID },
                    { key: 'data-plan-active', value: entry.isExpance },
                ],
                onClick: e => {
                    if (entry.categoryCode === 'C') {
        ...
        const entryBtn = button.build({
            ...
            callback: async () => {
                if (!entryBtn.classList.contains('disabled')) {
                    let accountName = filterValues.accountName == '%' ? undefined : filterValues.accountName;
                    NewEntryCF.init(accountName);
                }
            },
        });
    ...
    async function populateAccountDropdown() {
        const { getActiveAccountResult: accounts } = await ConsumerFinancesAjax.getActiveAccountAsync(
            selectedConsumersId,
            accountPermission,
        );
        ...
        if (data.length > 0) data.unshift({ id: null, value: '%', text: 'ALL' }); //ADD Blank value
    ...
    function filterButtonSet() {
        ...
        activityStartDateBtn = button.build({
            ...
            callback: () => {
                buildFilterPopUp('activityStartDateBtn');
            },
        });
        activityEndDateBtn = button.build({
            ...
            classNames: 'filterSelectionBtn',
            callback: () => {
                buildFilterPopUp('activityEndDateBtn');
            },
        });
        ...
        transectionTypeBtn = button.build({
            ...
            classNames: 'filterSelectionBtn',
            callback: () => {
                buildFilterPopUp('transectionTypeBtn');
            },
        });
        transectionTypeCloseBtn = button.build({
            ...
            callback: () => {
                closeFilter('transectionTypeBtn');
            },
        });
        accountNameBtn = button.build({
            ...
            classNames: 'filterSelectionBtn',
            callback: () => {
                buildFilterPopUp('accountNameBtn');
            },
        });
        ...
        isattachmentBtn = button.build({
            ...
            classNames: 'filterSelectionBtn',
            callback: () => {
                buildFilterPopUp('isattachmentBtn');
            },
        });
    ...
    function buildFilterPopUp(IsShow) {
        ...
        if (IsShow == 'ALL' || IsShow == 'activityStartDateBtn') dateWrap.appendChild(fromDateInput);
        if (IsShow == 'ALL' || IsShow == 'activityEndDateBtn') dateWrap.appendChild(toDateInput);

        var amountWrap = document.createElement('div');
        amountWrap.classList.add('dateWrap');
        if (IsShow == 'ALL' || IsShow == 'minamountBtn') amountWrap.appendChild(minAmountInput);
        if (IsShow == 'ALL' || IsShow == 'maxamountBtn') amountWrap.appendChild(maxAmountInput);

        // build popup
        filterPopup.appendChild(dateWrap);
        filterPopup.appendChild(amountWrap);
        if (IsShow == 'ALL' || IsShow == 'transectionTypeBtn') filterPopup.appendChild(transectionTypeFilterDropdown);
        if (IsShow == 'ALL' || IsShow == 'accountNameBtn') filterPopup.appendChild(accountFilterDropdown);
        if (IsShow == 'ALL' || IsShow == 'payeeBtn') filterPopup.appendChild(payeeDropdown);
        if (IsShow == 'ALL' || IsShow == 'categoryBtn') filterPopup.appendChild(categoryDropdown);
        if (IsShow == 'ALL' || IsShow == 'enteredByBtn') filterPopup.appendChild(lastUpdateDropdown);
        if (IsShow == 'ALL' || IsShow == 'isattachmentBtn') filterPopup.appendChild(isAttachedDropdown);
    ...
    function eventListeners() {
        ...
        maxAmountInput.addEventListener('keyup', event => {
            ...
            if (!reg.test(maxAmount)) {
                ...
                maxAmount.includes('.') &&
                (maxAmount.match(/\./g).length > 1 || maxAmount.toString().split('.')[1].length > 2)
            ) {
                document.getElementById('maxAmountInput').value = maxAmount.substring(0, maxAmount.length - 1);
                return;
            }
        ...
        lastUpdateDropdown.addEventListener('change', event => {
            tmpenteredBy =
                event.target.options[event.target.selectedIndex].innerHTML == 'ALL'
                    ? '%'
                    : event.target.options[event.target.selectedIndex].id; //event.target.options[event.target.selectedIndex].innerHTML;
    ...
    async function populateFilterDropdown() {
        ...
        const { getPayeesResult: Payees } = await ConsumerFinancesAjax.getPayeesAsync($.session.consumerId);
        ...
        let categoryID;
        const { getCategoriesSubCategoriesResult: Category } = await ConsumerFinancesAjax.getCategoriesSubCategoriesAsync(
            categoryID,
        );
        ...
        const { getActiveUsedByResult: employees } = await ConsumerFinancesAjax.getActiveEmployeesAsync();
