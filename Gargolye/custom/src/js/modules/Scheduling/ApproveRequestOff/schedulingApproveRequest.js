        ...
        if (daysOffRequests) {
            for (const request of daysOffRequests) {              
                ...
                if (decision !== '') {   
                    ...
                    var startTime = new Date(request.children[1].innerHTML).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
                    var endTime = new Date(request.children[2].innerHTML).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); 

                    if (startDate === endDate) {
                        dateTime = startTime == '12:00 AM' ? startDate : endTime == '11:59 PM' ? startDate + ' ' + startTime : startDate + ' ' + startTime + ' - ' + endTime ;
                        
                    }
                    else {
                        dateTime = startDate + ' ' + (startTime == '12:00 AM' ? '' : startTime) + ' - ' + endDate + ' ' + (endTime == '11:59 PM' ? '' : endTime);
                    }
