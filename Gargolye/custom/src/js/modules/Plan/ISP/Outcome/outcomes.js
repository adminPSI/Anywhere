const planOutcomes = (() => {
        ...
        o.planExperienceResponsibilities.forEach((resp, index) => {
          if (!oc.experiences[o.experienceId].responsibilities[index]) {
            oc.experiences[o.experienceId].responsibilities[index] = {
              ...
              responsibleProviderSalesforce: !resp.isSalesforceLocation ? '' : resp.isSalesforceLocation,
  ...
  async function populateResponsibleProviderDropdown(dropdownEle, defaultValue, defaultValueSalesForce) {
    const data = dropdownData.serviceVendors.map(dd => {
      return {
        value: dd.vendorId,
        text: dd.vendorName,
        salesforce: dd.isSalesforceLocation,
      };
    });
  ...
  async function insertOutcomeExperience(saveData, expTableWrap) {
    ...
    const tableOptions = {
      ...
      columnHeadings: ['What needs to happen', 'How should it happen?', 'Who is responsible?', 'When / How Often?'],
      sortable: false,
  ...
  function getColTextForWhoResponsible(responsibleContact, responsibleProvider, responsibleProviderSalesforce) {
    if (responsibleProvider !== '%' && responsibleProvider !== '0') {
      const filteredVendor = dropdownData.serviceVendors.filter(dd => dd.vendorId === responsibleProvider);

      if (!filteredVendor.length) {
        return '';
      }

      if (filteredVendor.length === 1) {
      ...
      if (dropdownData.relationships) {
        const filteredRelationship = dropdownData.relationships.filter(dd => dd.contactId === responsibleContact);
  ...
  function showExperiencesPopup(data, isNew) {
    ...
    function buildResponsiblePartyInputs(saveUpdateDataRef, dataRefKey) {
      ...
      wrapHeading.innerHTML = '<h2>*Who Is Responsible?</h2><p>Choose either a Responsible Person or Provider.</p>';
      ...
      const whenHowOftenFrequencyDropdown = dropdown.build({
        ...
        callback: (e, selectedOption) => {
          ...
          if (saveUpdateDataRef.whenHowOftenFrequency === '' || saveUpdateDataRef.whenHowOftenFrequency === '%') {
      ...
      //* INIT REQUIRED FIELDS
      //*--------------------------------
      if (saveUpdateDataRef.responsibleContact === '%' && saveUpdateDataRef.responsibleProvider === '%') {
        whoReponsibleWrap.classList.add('error');
        responsiblePersonDropdown.classList.add('error');
        responsibleProviderDropdown.classList.add('error');
        hasInitialErros = true;
      ...
      populateExperiencesHowOftenDropdown(whenHowOftenFrequencyDropdown, saveUpdateDataRef.whenHowOftenFrequency);
      populateResponsibleProviderDropdown(
        responsibleProviderDropdown,
        saveUpdateDataRef.responsibleProvider,
        saveUpdateDataRef.responsibleProviderSalesforce,
      );
      planData.populateRelationshipDropdown(responsiblePersonDropdown, saveUpdateDataRef.responsibleContact);
    ...
    const addRespPartyBtn = button.build({
      ...
      callback: () => {
        ...
        // build & append it
        const respParty = buildResponsiblePartyInputs(saveUpdateData.responsibilities[respLength], respLength);
        respPartyWrap.appendChild(respParty);
        toggleExperiencesPopupDoneBtn();
      },
    });
  ...
  function buildExperiencesTables(outcomeId) {
    ...
    if (outcomesData[outcomeId] && outcomesData[outcomeId].experiences) {
      Object.values(outcomesData[outcomeId].experiences)
        .sort((a, b) => {
          return a.experienceOrder < b.experienceOrder ? -1 : a.experienceOrder > b.experienceOrder ? 1 : 0;
        })
        .map((td, index) => {
          const tableData = [];
          ...
          Object.values(td.responsibilities).forEach((resp, index) => {
            ...
            const responsibleProviderSalesforce = resp.responsibleProviderSalesforce;
            const whenHowOftenValue = resp.whenHowOftenValue;
            ...
            respObj[index] = {
              ...
              responsibleContact: responsibleContact,
              responsibleProvider: responsibleProvider,
              responsibleProviderSalesforce: responsibleProviderSalesforce,
              whenHowOftenValue: whenHowOftenValue,
              whenHowOftenFrequency: whenHowOftenFrequency,
              whenHowOftenText: whenHowOftenText,
            };
  ...
  function buildReviewsTable(outcomeId) {
    ...
    const reviewsTable = table.build({
      ...
      headline: 'Reviews',
      columnHeadings: [`What will progress look like? How will we know it's happening?`, 'Who?', 'When to check In?'],
      sortable: isSortable,
  ...
  function buildOutcome({ outcomeId, ...outcomeData }) {
    ...
    carryoverInput.addEventListener('input', e => {
      ...
      if (hasPreviousPlan && outcomesUpdateData.status === '1' && outcomesUpdateData.carryOverReason === '') {
        carryoverInput.classList.add('error');
      } else {
        carryoverInput.classList.remove('error');
      }
    ...
    const historyInput = buildQuestion({
      ...
      prompt: 'Remember to only include history that may impact a persons life, support and achievement of outcomes.',
  ...
  async function init(data) {
    ...
    if (!planOutcomesData.planProgressSummary || planOutcomesData.planProgressSummary.length === 0) {
